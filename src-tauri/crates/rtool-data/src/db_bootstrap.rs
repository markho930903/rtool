...
async fn ensure_schema_migrations_table(conn: &DbConn) -> DbResult<()> {
    conn.execute_batch(
        r#"
...
async fn has_schema_migration(conn: &DbConn, version: i64) -> DbResult<bool> {
    let mut rows = conn
        .query(
            "SELECT EXISTS(
...
async fn record_schema_migration(conn: &DbConn, version: i64, name: &str) -> DbResult<()> {
    conn.execute(
...
async fn migrate_add_preview_path(conn: &DbConn) -> DbResult<()> {
...
async fn migrate_add_preview_data_url(conn: &DbConn) -> DbResult<()> {
    if let Err(error) = conn
        .execute(
...
async fn migrate_add_content_key(conn: &DbConn) -> DbResult<()> {
    if let Err(error) = conn
        .execute(
...
async fn apply_schema_migrations(conn: &DbConn) -> DbResult<()> {
    ...
    if !has_schema_migration(conn, SCHEMA_VERSION_ADD_PREVIEW_PATH).await? {
        migrate_add_preview_path(conn).await?;
        record_schema_migration(conn, SCHEMA_VERSION_ADD_PREVIEW_PATH, "add_preview_path").await?;
    }

    if !has_schema_migration(conn, SCHEMA_VERSION_ADD_PREVIEW_DATA_URL).await? {
        migrate_add_preview_data_url(conn).await?;
        record_schema_migration(
            conn,
            SCHEMA_VERSION_ADD_PREVIEW_DATA_URL,
            "add_preview_data_url",
        )
        .await?;
    }

    if !has_schema_migration(conn, SCHEMA_VERSION_ADD_CONTENT_KEY).await? {
        migrate_add_content_key(conn).await?;
...
pub async fn init_db(conn: &DbConn) -> DbResult<()> {
    ...
    conn.execute_batch(
        r#"
        DROP INDEX IF EXISTS idx_transfer_sessions_created_at;
        DROP INDEX IF EXISTS idx_transfer_sessions_status;
        DROP INDEX IF EXISTS idx_transfer_sessions_cleanup;
        DROP INDEX IF EXISTS idx_transfer_files_session_id;
        DROP INDEX IF EXISTS idx_transfer_peers_last_seen;
        DROP TABLE IF EXISTS transfer_files;
        DROP TABLE IF EXISTS transfer_sessions;
        DROP TABLE IF EXISTS transfer_peers;
        "#,
    )
    .await?;
