...
const SIDEBAR_ITEM_ACTIVE_CLASS = "bg-surface-glass text-text-primary [&_.sidebar-item-label]:text-text-primary";

const SIDEBAR_ITEM_IDLE_CLASS =
  "text-text-secondary hover:bg-surface-soft hover:text-text-primary hover:[&_.sidebar-item-label]:text-text-primary";

const TITLEBAR_ICON_BUTTON_CLASS =
  "ui-glass-hover inline-flex h-9 w-9 select-none items-center justify-center rounded-3 border border-transparent text-text-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/45";

const TITLEBAR_MENU_TRIGGER_BUTTON_CLASS =
  "ui-glass-hover inline-flex h-9 max-w-[12rem] select-none items-center gap-1.5 rounded-3 border border-transparent px-2.5 text-text-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/45";
...
const TITLEBAR_MENU_ITEM_ACTIVE_CLASS = "bg-surface-glass text-text-primary";

const TITLEBAR_MENU_ITEM_IDLE_CLASS = "hover:bg-surface-soft hover:text-text-primary";
...
function SideBar({ pathname }: { pathname: string }) {
  ...
  return (
    <aside className="rtool-glass-sheen-clip z-20 flex h-full w-[84px] shrink-0 flex-col items-center overflow-hidden border-r border-layout-divider bg-layout-sidebar">
      ...
      <nav className="mt-1 flex flex-1 flex-col items-center gap-2 py-2" aria-label={t("nav.mainAria")}>
        {NAV_ITEMS.map((item) => {
          ...
          const isActive = isRouteActiveById(item.id, pathname);
          return (
            <NavLink
              key={item.to}
              to={item.to}
              end={item.end}
              className={[SIDEBAR_ITEM_BASE_CLASS, isActive ? SIDEBAR_ITEM_ACTIVE_CLASS : SIDEBAR_ITEM_IDLE_CLASS].join(
                " ",
              )}
              title={label}
              aria-label={label}
...
export default function AppLayout() {
  ...
  const menuPopup = menuOpen
    ? createPortal(
        <div className="fixed inset-0 z-[70]" onPointerDown={closeMenu}>
          <div
            ref={menuRef}
            className="fixed z-[80] overflow-hidden rounded-md border border-border-muted bg-surface-glass-strong p-1.5 shadow-surface backdrop-blur-[var(--glass-blur)] backdrop-saturate-[var(--glass-saturate)] backdrop-brightness-[var(--glass-brightness)]"
            style={{
          ...
          >
            <div className="overflow-y-auto" style={{ maxHeight: `${menuPosition.maxHeight}px` }}>
              <nav className="flex w-full flex-col items-stretch gap-1" aria-label={t("titlebar.menuAria")}>
                {NAV_ITEMS.map((item) => {
                  const label = t(item.labelKey);
                  const isActive = isRouteActiveById(item.id, location.pathname);
                  return (
                    <NavLink
                      key={item.to}
                      to={item.to}
  ...
  if (layoutPreference === "sidebar") {
    return (
      <div className="relative h-full w-full overflow-hidden bg-transparent p-0 text-text-primary">
        <section className="rtool-glass-sheen-clip relative z-10 flex h-full w-full overflow-hidden bg-layout-titlebar shadow-surface backdrop-blur-[var(--glass-blur)] backdrop-saturate-[var(--glass-saturate)] backdrop-brightness-[var(--glass-brightness)]">
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-transparent p-0 text-text-primary">
      <section className="rtool-glass-sheen-clip relative z-10 flex h-full w-full flex-col overflow-hidden bg-layout-titlebar shadow-surface backdrop-blur-[var(--glass-blur)] backdrop-saturate-[var(--glass-saturate)] backdrop-brightness-[var(--glass-brightness)]">
        <div aria-hidden className="rtool-glass-atmosphere" />
        <header className="rtool-glass-sheen-open relative z-20 flex h-12 shrink-0 items-center border-b border-layout-divider bg-layout-titlebar">
          <div className="relative flex h-full items-center pl-24 pr-2">
