...
use crate::transfer::TRANSFER_LISTEN_PORT;
use crate::transfer::archive::{cleanup_temp_paths, collect_sources};
use crate::transfer::discovery::{DiscoveryPacket, PeerMap, run_broadcast_loop, run_listen_loop};
use crate::transfer::preview::build_preview;
...
impl TransferService {
    ...
    pub async fn clear_history(&self, input: TransferClearHistoryInputDto) -> AppResult<()> {
        ...
        crate::transfer::store::clear_history(&self.db_conn, all, older_than_days).await?;
...
async fn resolve_or_create_device_id(conn: &DbConn) -> AppResult<String> {
    ...
    if let Some(value) = rtool_db::db::get_app_setting(conn, key).await? {
        let trimmed = value.trim();
        if !trimmed.is_empty() {
    ...
    rtool_db::db::set_app_setting(conn, key, value.as_str()).await?;
    Ok(value)
}
...
mod tests {
    ...
    async fn create_service(
        ...
        let db_path = app_data_dir
            ...
            .to_string_lossy()
            .to_string();
        let db_conn = rtool_db::db::open_db(Path::new(db_path.as_str())).await?;
        rtool_db::db::init_db(&db_conn).await?;
        TransferService::new(
