...
export const Radio = forwardRef<HTMLInputElement, RadioProps>(function Radio(props, ref) {
  ...
  const inputClassName = cx(
    "m-0 shrink-0 rounded-full border border-border-strong bg-surface-soft text-accent accent-accent",
    "outline-none focus-visible:ring-2 focus-visible:ring-accent-soft focus-visible:ring-offset-2 focus-visible:ring-offset-app",
    sizeClassMap[size],
  ...
  return (
    <label
      className={cx(
        isCard
          ? "inline-flex min-h-10 items-center gap-2 overflow-visible rounded-md border border-border-strong bg-surface-soft px-3 py-2 text-xs text-text-secondary shadow-inset-soft transition-[background-color,border-color,color] duration-150 hover:border-accent/45 hover:bg-surface"
          : "inline-flex items-start gap-2 text-sm text-text-secondary",
        isCard && checked ? "border-accent bg-accent-soft text-text-primary" : null,
