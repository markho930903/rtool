...
pub(crate) fn refresh_window_titles<R: Runtime>(app: &AppHandle<R>, locale: &str) {
    ...
    for (label, title_key) in [
        ...
        (SCREENSHOT_WINDOW_LABEL, "window.screenshot.title"),
    ] {
        let Some(window) = app.get_webview_window(label) else {
            continue;
        };

        let title = locale_service.translate(locale, title_key);
    ...
    for label in SCREENSHOT_PIN_WINDOW_LABELS {
        let Some(window) = app.get_webview_window(label) else {
            continue;
        };

        let title = locale_service.translate(locale, "window.screenshotPin.title");
        if let Err(error) = window.set_title(&title) {
            tracing::warn!(
                event = "window_title_update_failed",
                window = label,
