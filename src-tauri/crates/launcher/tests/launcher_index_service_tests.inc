...
use rtool_db::db::{DbConn, get_app_setting, init_db, open_db, set_app_setting};
use uuid::Uuid;
...
fn truncation_log_classification_should_be_expected_for_default_profile() {
    let default_settings = LauncherSearchSettingsRecord::default().normalize();
    assert_eq!(
        classify_truncation_log_level(&default_settings),
