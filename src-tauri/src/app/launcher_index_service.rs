...
const SEARCH_SETTINGS_KEY: &str = "launcher.search.settings.v2";
...
fn load_or_init_settings(db_pool: &DbPool) -> AppResult<LauncherSearchSettingsRecord> {
    ...
    let Some(raw_value) = get_app_setting(db_pool, SEARCH_SETTINGS_KEY)? else {
...
fn default_search_roots() -> Vec<String> {
    #[cfg(target_os = "windows")]
    {
        ...
        for letter in b'A'..=b'Z' {
            ...
            if Path::new(root.as_str()).exists() {
                roots.push(root);
            }
        }
        if roots.is_empty()
