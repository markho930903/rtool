...
use foundation::transfer::TRANSFER_LISTEN_PORT;
use foundation::transfer::archive::{cleanup_temp_paths, collect_sources};
use foundation::transfer::discovery::{
    DiscoveryPacket, PeerMap, run_broadcast_loop, run_listen_loop,
};
use foundation::transfer::preview::build_preview;
use foundation::transfer::protocol::{
...
impl TransferService {
    ...
    pub async fn clear_history(&self, input: TransferClearHistoryInputDto) -> AppResult<()> {
        ...
        foundation::transfer::store::clear_history(&self.db_conn, all, older_than_days).await?;
...
async fn resolve_or_create_device_id(conn: &DbConn) -> AppResult<String> {
    ...
    if let Some(value) = foundation::db::get_app_setting(conn, key).await? {
        let trimmed = value.trim();
        if !trimmed.is_empty() {
    ...
    foundation::db::set_app_setting(conn, key, value.as_str()).await?;
    Ok(value)
}
...
mod tests {
    ...
    async fn create_service(
        ...
        let db_path = app_data_dir
            ...
            .to_string_lossy()
            .to_string();
        let db_conn = foundation::db::open_db(Path::new(db_path.as_str())).await?;
        foundation::db::init_db(&db_conn).await?;
        TransferService::new(
