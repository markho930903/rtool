...
export default function ClipboardWindowPage() {
  ...
  useAsyncEffect(
    ...
    [appWindow, applyModeSize, enabled, syncLocaleFromBackend],
    {
      scope: "clipboard-window",
      onError: (error) => {
        if (import.meta.env.DEV) {
  ...
  return (
    <div className="h-full w-full overflow-hidden bg-transparent p-0 text-text-primary">
      <main
        className="rtool-glass-sheen-clip flex h-full w-full overflow-hidden bg-layout-titlebar p-0 shadow-surface backdrop-blur-[var(--glass-blur)] backdrop-saturate-[var(--glass-saturate)] backdrop-brightness-[var(--glass-brightness)]"
