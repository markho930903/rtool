...
fn has_missing_clipboard_content_keys(conn: &Connection) -> AppResult<bool> {
...
fn has_duplicate_clipboard_content_keys(conn: &Connection) -> AppResult<bool> {
    let exists = conn.query_row(
        "SELECT EXISTS(
            SELECT 1 FROM (
                SELECT content_key
...
pub fn init_db(db_path: &Path) -> AppResult<()> {
    ...
    if has_missing_clipboard_content_keys(&conn)? {
        backfill_clipboard_content_keys(&conn)?;
    }
    if has_duplicate_clipboard_content_keys(&conn)? {
