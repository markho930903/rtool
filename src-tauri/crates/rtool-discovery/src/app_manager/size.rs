...
#[derive(Debug, Clone)]
pub(super) struct AppSizePathResolution {
    pub(super) path: PathBuf,
    pub(super) size_source: AppManagerSizeSource,
}
...
fn resolve_bundle_ancestor(path: &Path) -> Option<PathBuf> {
    #[cfg(target_os = "macos")]
    {
...
fn resolve_bundle_size_path(path: &Path) -> Option<AppSizePathResolution> {
    resolve_bundle_ancestor(path).map(|bundle_path| AppSizePathResolution {
        path: bundle_path,
        size_source: AppManagerSizeSource::AppBundle,
    })
}

pub(super) fn resolve_app_size_path(path: &Path) -> AppSizePathResolution {
    if let Some(resolution) = resolve_bundle_size_path(path) {
        return resolution;
    }
    ...
    if path.is_file() {
        return AppSizePathResolution {
            ...
            size_source: AppManagerSizeSource::ParentDirectory,
        };
    }
...
pub(super) fn resolve_self_app_size_path(path: &Path) -> AppSizePathResolution {
    if let Some(resolution) = resolve_bundle_size_path(path) {
        return resolution;
    }
    AppSizePathResolution {
...
pub(super) fn resolve_managed_app_size_path(item: &ManagedAppDto) -> AppSizePathResolution {
    let path = Path::new(item.path.as_str());
    if matches!(item.source, AppManagerSource::Rtool) {
        return resolve_self_app_size_path(path);
    }
    resolve_app_size_path(path)
}
