...
const triggerClassMap: Record<SelectVariant, string> = {
  default:
    "w-full rounded-md border border-border-strong bg-surface-soft px-3 text-sm text-text-primary shadow-inset-soft outline-none transition-[border-color,background-color,box-shadow] duration-150 hover:border-accent/55 hover:bg-surface",
  tool: "rounded-md border border-border-strong bg-surface-soft px-2.5 text-ui-sm leading-ui-sm text-text-primary shadow-inset-soft outline-none transition-[border-color,background-color,box-shadow] duration-150 hover:border-accent/55 hover:bg-surface",
  clipboard:
    "rounded-md border border-border-strong bg-surface-soft px-2.5 text-ui-sm leading-ui-sm text-text-primary shadow-inset-soft outline-none transition-[border-color,background-color,box-shadow] duration-150 hover:border-accent/55 hover:bg-surface",
  theme:
    "h-8 rounded-md border border-border-strong bg-surface-soft px-2 text-xs text-text-secondary shadow-inset-soft outline-none transition-[border-color,background-color,box-shadow] duration-150 hover:border-accent/55 hover:bg-surface",
};

const panelClassMap: Record<SelectVariant, string> = {
  default:
    "overflow-y-auto rounded-md border border-border-strong bg-surface p-1 shadow-overlay backdrop-blur-[var(--glass-blur)] backdrop-saturate-[var(--glass-saturate)] backdrop-brightness-[var(--glass-brightness)]",
  tool: "overflow-y-auto rounded-md border border-border-strong bg-surface p-1 shadow-overlay backdrop-blur-[var(--glass-blur)] backdrop-saturate-[var(--glass-saturate)] backdrop-brightness-[var(--glass-brightness)]",
  clipboard:
    "overflow-y-auto rounded-md border border-border-strong bg-surface p-1 shadow-overlay backdrop-blur-[var(--glass-blur)] backdrop-saturate-[var(--glass-saturate)] backdrop-brightness-[var(--glass-brightness)]",
  theme:
    "overflow-y-auto rounded-md border border-border-strong bg-surface p-1 shadow-overlay backdrop-blur-[var(--glass-blur)] backdrop-saturate-[var(--glass-saturate)] backdrop-brightness-[var(--glass-brightness)]",
...
const optionClassMap: Record<SelectVariant, string> = {
  default: "min-h-[34px] rounded-sm border border-transparent px-2.5 text-sm",
  tool: "min-h-[32px] rounded-sm border border-transparent px-2.5 text-ui-sm leading-ui-sm",
  clipboard: "min-h-[32px] rounded-sm border border-transparent px-2.5 text-ui-sm leading-ui-sm",
  theme: "min-h-[30px] rounded-sm border border-transparent px-2 text-xs",
};
...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(function Select(props, ref) {
  ...
  if (multiple) {
    const selectClassName = cx(
      ...
      invalid ? "border-danger focus-visible:border-danger focus-visible:ring-danger/20" : null,
                ...
                <li
                  ...
                  className={cx(
                    ...
                    option.disabled
                      ...
                      : "cursor-pointer text-text-secondary hover:border-border-strong hover:bg-surface-soft hover:text-text-primary",
                    isActive && !option.disabled ? "border-border-strong bg-surface-soft text-text-primary" : null,
                    isSelected && !option.disabled ? "border-accent/40 bg-accent-soft text-text-primary" : null,
                  )}
                  onMouseEnter={() => {
                    if (!option.disabled) {
